		...
		<table class="table table-hover table-striped table-borderless" style="border-collapse: separate; border-spacing: 0;">
			<thead class="sticky-top bg-white">
				<tr>
					...
					<th></th>
					<th>
						<form action="<?= (base_url('firstname/search')) ?>" method="post">
							<div>
			...
			<tbody>
				<?php foreach ($session->firstnames as $firstname): ?>
						<tr>
							<td><?= esc($firstname['Firstname'])?></th>
							<td><?= esc($firstname['Firstname_popularity'])?></td>
							<td>
								<label for="next_action" class="sr-only">Next action</label>
									<select class="box" name="next_action" id="next_action">
										<?php foreach ($session->transcription_cycles as $key => $transcription_cycle): ?>
											<?php if ( $transcription_cycle['BMD_cycle_type'] == 'FIRNA' ): ?>
							...
							<td>
								<button  
									data-id="<?= esc($firstname['Firstname']); ?>" 
									class="go_firstname_button btn btn-success btn-sm">Go
								</button>
							</td>
						</tr>
				<?php endforeach; ?>
	...
	<div>
		<form action="<?= (base_url('firstname/next_action')); ?>" method="POST" name="form_next_action" >
