	...
	<form action="<?php echo(base_url('identity/retrieve_password_step2')) ?>" method="post">
		...
		<div class="form-group row">
			...
			<small id="userHelp" class="form-text text-muted"><?php echo 'This must be your '.$session->current_project['project_name'].' identity.' ?></small>
		</div>
	  
		<div class="form-group row">
				<label for="email">Your email</label>
				<input type="email" class="form-control" id="email" name="email" value="<?php echo($session->email) ?>">
				<small id="userHelp" class="form-text text-muted"><?php echo 'This must be the email address attached to your '.$session->current_project['project_name'].' identity.' ?></small>
		</div>
